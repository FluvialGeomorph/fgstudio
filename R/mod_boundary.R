...
mod_boundary_ui <- function(id, exploration_only = FALSE, selection_draft = NULL, study = NULL) {
  ...
  bslib::card(
    bslib::card_header(if (exploration_only) "Explore before defining your study" else
      if (isTRUE(study$boundary)) "2. Study Area and Stream geometry" else "2. Define your Study Area"),
    shiny::radioButtons(ns("selection_target"), "Working on",
      choices = if (exploration_only) c("Study Area" = "boundary") else
        c("View" = "view", "Study Area" = "boundary", "Streams" = "stream"),
    ...
    shiny::div(style = "display:none;", shiny::radioButtons(ns("map_mode"), NULL,
      choices = c("view", "draw", "explore"), selected = boundary_initial_mode(study, exploration_only, selection_draft))),
    shiny::uiOutput(ns("task_status")),
    bslib::layout_columns(col_widths = bslib::breakpoints(sm = c(12, 12), lg = c(7, 5)),
      shiny::div(
        shiny::div(style = "height: clamp(300px, 54vh, 620px);",
      ...
      shiny::div(
        shiny::conditionalPanel("input.map_mode === 'view'", ns = ns,
          compact_table(data.frame(Item = c("Boundary", "Next", "Editing rule"),
            Status = c("Saved", "Choose Study Area to edit, or Streams to add a Stream", "Must retain all saved Stream and Reach areas")))),
...
mod_boundary_server <- function(id, study, store, is_active, on_saved, search = search_places,
  ...
  shiny::moduleServer(id, function(input, output, session) {
    ...
    output$map <- leaflet::renderLeaflet({
      ...
      if (inherits(study$stream_inventory, "sf")) map <- leaflet::addPolygons(map,
        data = sf::st_transform(study$stream_inventory, 4326), group = "Saved Streams",
        color = "#00796b", weight = 3, fillOpacity = .15, label = study$stream_inventory$stream_name,
    ...
    layer_groups <- shiny::reactive(context_layer_groups(input$map_mode, input$selection_target,
    ...
    output$workflow_next <- shiny::renderUI({
      ...
      step <- if (stream && !isTRUE(study$boundary)) "Choose Study Area and save its boundary first." else
        if (stream && !is.null(stream_selection$pool()) && identical(input$stream_action, "select")) {
          if (!length(stream_selection$selected())) "Check the channel segments in the lists below (or click their lines)." else
    ...
    drawing <- shiny::observeEvent(input$map_draw_all_features, {
      if (!is_active() || exploration_only || isTRUE(input$map_mode %in% c("explore", "view"))) return()
      dirty(TRUE)
      candidate(NULL)
      tryCatch({
    ...
    list(has_unsaved = function() shiny::isolate(dirty()) || selection$has_pending() || stream_selection$has_pending(),
      selection_state = function() shiny::isolate(c(selection$state(), list(
        ...
        distance = input$navigation_km, stream = stream_selection$state()))), destroy = function() {
...
boundary_initial_mode <- function(study, exploration_only, draft) {
  ...
  if (!is.null(draft$map_mode) && draft$map_mode %in% c("view", "draw", "explore")) return(draft$map_mode)
